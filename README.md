...
LocoBuffer-USB exposed by `ser2net`, a hardware bridge running in TCP-bridge
mode, or JMRI's `LocoNetOverTcp` server — from any platform .NET runs on
(Windows, Linux, macOS, containers).

This is a managed port of the [mrrwa/LocoNet2](https://github.com/mrrwa/LocoNet2)
Arduino library. This repository now keeps only the .NET implementation; original
project credits are preserved in [`CREDITS.md`](CREDITS.md).
...
| `LocoNet.Net.Tests` | MSTest suite — 134 tests including loopback TCP resilience tests. |
