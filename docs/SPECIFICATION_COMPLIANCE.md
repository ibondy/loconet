...
(raw-binary and JMRI ASCII) rather than the Arduino UART pathways. The port mirrors the original LocoNet2 feature set documented
above; the following audit-pass changes were applied to bring it to parity and fix latent bugs:

| Area | Change | Status |
|---|---|---|
| `OpCode` enum | Removed duplicate `AnalogIo = 0xE5` (kept `PeerXfer`); both share the byte and are disambiguated by the message body. | Fixed |
| `LocoNetThrottle.SetSpeed` | Defer condition corrected to `speed > 1 || _ticksSinceLastAction == 0` to mirror the original tick-refresh behavior. | Fixed |
...
JMRI ASCII framing. Coverage is verified by 134 MSTest tests in `LocoNet.Net.Tests`.
